...
| [`mcp-research-agent`](https://github.com/tuvl-io/examples/tree/release/mcp-research-agent) | Medium–Complex | An `AutonomousAgent` driving an MCP fetch tool to a cited research brief, bounded by a token budget, with live loop progress over the SDK |
...
Together the five exercise every step kind and subsystem: all nine step kinds,
RAG, `spec.supervisor`, `HumanInTheLoop` with `auth.required_group`, MCP over
stdio, model versioning, `secure: true` masking, IAM scopes and groups, and the
`tuvl test` framework with LLM-judge evaluations.
