...
| Date | Change |
|---|---|
| 2026-08-03 | §1 — version badge `1.0.1`. **Patch — correctness fixes only, no API/schema change.** Default-less nullable `uuid`/`date`/`timestamp`/`timestamptz` model fields are no longer silently auto-generated on ModelOp CRUD create (a nullable uuid FK got a dangling UUID; a date/timestamp got `today`/`now()` instead of `NULL`). Auto-generation is now limited to a `uuid` **primary key** and the `created_at`/`updated_at` audit-column convention; opt any other field in explicitly with `default: uuid4 \| now \| today`. CLI commands (e.g. `tuvl --version`) no longer print the `Settings loaded` startup log. |
